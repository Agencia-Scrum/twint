...
def test_search():
    ...
    config.Custom_query = '(gerardo morales OR gerardomorales OR gobernador jujuy OR gerardopresidente) since:2023-03-15 until:2023-03-17'
    config.Limit = 20
    config.Store_object = True
    config.Hide_output = True
    twint.run.Search(config)
    tweets_mios = twint.output.tweets_list
    tweets_mios[0].impressions_count
    
    tweets_con_impr = []
    tweets_sin_impr = []
    for tweet in tweets_mios:
        if hasattr(tweet,'impressions_count'):
            tweets_con_impr.append(tweet)
        else:
            tweets_sin_impr.append(tweet)
    print(len(tweets_con_impr))
    print(len(tweets_sin_impr))
    return tweets_mios, tweets_con_impr, tweets_sin_impr            
